...
class Secret_Santa(commands.Cog, name="Secret Santa"):
    """Secret Santa event!"""
    ...
    async def start(self, ctx):
        event_role = discord.utils.get(ctx.guild.roles, name="Secret Santa") or discord.utils.get(ctx.guild.roles, name="secret santa")
        
        if event_role is None:
            await ctx.send("There is no role named either \"Secret Santa\" or \"secret santa\". Please create a role with either of these names before trying again.")
            return
        ...
        if not event_members:
            await ctx.send(f"It doesn't seem like anyone has the {event_role.mention} role, you might want to invite some people first!")
            return
        elif len(event_members) <= 2:
            await ctx.send("You know you don't need to hold a secret santa for two people to give each other gifts right?")
            return
