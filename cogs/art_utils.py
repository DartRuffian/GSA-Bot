...
class Art_Utils(commands.Cog, name="Artist Utilities"):
    """ Cog description """
    ...
    async def remove_background(self, ctx, image_link=None):
        if not image_link and not ctx.message.attachments:
            await ctx.message.reply("Make sure to either send an image link or upload an image in your message!", mention_author=False)
            return

        await ctx.send("Processing your image, this may take a while... You'll be pinged when your image is done, or if an error occurred.")
